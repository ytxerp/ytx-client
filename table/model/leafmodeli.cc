...
bool LeafModelI::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    case EntryEnumI::kDocument:
        ...
        break;
    case EntryEnumI::kIsChecked:
        EntryUtils::UpdateShadowField(caches_[id], shadow, kIsChecked, value.toBool(), &EntryShadow::is_checked, [id, this]() { RestartTimer(id); });
        break;
    case EntryEnumI::kDescription:
...
bool LeafModelI::UpdateRate(EntryShadow* entry_shadow, double value)
{
    ...
    *d_shadow->unit_cost = value;
    ...
    if (has_leaf_delta) {
        QJsonObject lhs_delta { JsonGen::LeafDelta(lhs_id_, 0.0, lhs_final_delta) };
        QJsonObject rhs_delta { JsonGen::LeafDelta(*d_shadow->rhs_node, 0.0, rhs_final_delta) };
...
bool LeafModelI::UpdateRhsNode(EntryShadow* entry_shadow, const QUuid& value, int row)
{
    if (value.isNull())
        return false;
...
bool LeafModelI::removeRows(int row, int /*count*/, const QModelIndex& parent)
{
    ...
    if (!rhs_node_id.isNull()) {
        ...
        if (has_delta) {
            QJsonObject lhs_delta { JsonGen::LeafDelta(lhs_id_, lhs_initial_delta, lhs_final_delta) };
            QJsonObject rhs_delta { JsonGen::LeafDelta(rhs_node_id, rhs_initial_delta, rhs_final_delta) };
